...
//navbar link active and data section selection...
...
if (dateMonth > 9 ) {
    ...
    if (dateDay > 9) {
        dateTostring = `${dateDay}-${dateMonth}-${date.getFullYear()}`;
    }
}
if (dateDay > 9) {
        ...
        .then(response => {
            ...
            for (let prayerData of prayer.data) {
                ...
                if (prayerData.date.gregorian.date === dateTostring) {
                    ...
                    let hijriDate = `<span> Hijri : ${prayerData.date.hijri.date}</span>`;
                    document.querySelector(".hijri-date").innerHTML=hijriDate;
